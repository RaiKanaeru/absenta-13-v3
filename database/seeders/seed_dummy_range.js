...
const getTargetDays = (includeSaturdays) =>
  includeSaturdays
...
const buildConnectionConfig = () => {
  ...
  if (SEED_DB.password !== undefined) {
    config.password = SEED_DB.password;
  }
...
async function loadReferenceData(connection) {
  ...
  return {
    mapelFinal,
    kelasNameSet: new Set(kelasExisting.map((row) => row.nama_kelas)),
    ruangCodeSet: new Set(ruangExisting.map((row) => row.kode_ruang)),
    maxGuruId: Number(maxGuruRows[0]?.max_id || 0),
    maxSiswaId: Number(maxSiswaRows[0]?.max_id || 0),
  };
}

async function seedJamPelajaran(connection, config, includeSaturdays, tahunAjaran) {
  const jamRows = [];
  for (const [hari, slots] of Object.entries(config)) {
...
async function seedRuangKelas(connection, classCount, existingSet) {
  const ruangRows = [];
...
async function seedKelas(connection, classCount, runTag, existingSet) {
  const kelasRows = [];
  ...
  for (let i = 0; i < classCount; i += 1) {
    ...
    if (nama.length > 50 || existingSet.has(nama)) continue;
...
async function seedGuruAvailability(connection, guruRows, includeSaturdays, tahunAjaran) {
  const availabilityRows = [];
  const availabilityDays = getTargetDays(includeSaturdays);

  for (const guru of guruRows) {
    const idGuru = guru[0];
    for (const hari of availabilityDays) {
      const isAvailable = Math.random() < 0.9 ? 1 : 0;
      availabilityRows.push([idGuru, hari, isAvailable, isAvailable ? 'OK' : 'Tidak tersedia', tahunAjaran]);
    }
  }
...
async function seedGuruUsersAndData(connection, teacherCount, runTag, maxGuruId, mapelFinal) {
  const guruUserRows = [];
  ...
  await seedGuruAvailability(connection, guruRows, CONFIG.includeSaturdays, CONFIG.tahunAjaran);
  return { guruRows, guruMapel };
}

async function updateKelasJumlahSiswa(connection, kelasFinal, siswaByClass) {
...
async function seedSiswaUsersAndData(connection, kelasFinal, studentsPerClass, runTag, maxSiswaId) {
  const siswaUserRows = [];
  const siswaPlan = [];
  let siswaIndex = 0;
  ...
  await updateKelasJumlahSiswa(connection, kelasFinal, siswaByClass);
  return { siswaByClass };
}

async function syncMataPelajaran(connection, mapelFinal) {
...
const buildRuangMapByKelas = (kelasFinal, ruangFinal) => {
...
async function seedJadwalGuru(connection, jadwalFinal) {
...
async function seedRuangMapelBinding(connection, mapelFinal, ruangFinal) {
  const bindingRows = [];
...
async function seedJadwalAndRelated(connection, kelasFinal, mapelFinal, guruMapel, ruangFinal, targetDays, jamByDay) {
  const jadwalRows = [];
  const ruangMap = buildRuangMapByKelas(kelasFinal, ruangFinal);
  const fallbackGuruId = [...guruMapel.values()][0]?.[0];
  ...
  kelasFinal.forEach((kelas, kelasIndex) => {
    for (const hari of targetDays) {
      ...
      slots.forEach((slot, slotIndex) => {
        ...
        const guruId = guruList.length ? randomItem(guruList) : fallbackGuruId;
        jadwalRows.push([
          kelas.id_kelas,
          mapel.id_mapel,
          guruId,
  ...
  await seedJadwalGuru(connection, jadwalFinal);
  await seedRuangMapelBinding(connection, mapelFinal, ruangFinal);

  return { jadwalFinal, kelasIdsList };
}

const buildJadwalByClassDay = (jadwalFinal) => {
  ...
  return jadwalByClassDay;
};

const isSchoolDay = (date, includeSaturdays) => {
  const dayName = DAY_NAMES[date.getDay()];
  if (includeSaturdays) {
    return dayName !== 'Minggu';
  }
  return ['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat'].includes(dayName);
};

const generateAbsensiForSchedule = (sched, siswaList, dateStr, statusGuru, siswaPencatat) => {
  ...
  const siswaRecords = siswaList.map((siswaId) => {
  ...
  return { guruRecord, siswaRecords };
};

async function batchInsertAbsensi(connection, siswaBatch, guruBatch) {
  if (siswaBatch.length) {
    await chunkInsert(connection, 'absensi_siswa', ABSENSI_SISWA_COLUMNS, siswaBatch);
  }
  if (guruBatch.length) {
    await chunkInsert(connection, 'absensi_guru', ABSENSI_GURU_COLUMNS, guruBatch);
  }
}
...
async function generateAbsensiData(connection, startDate, endDate, kelasFinal, jadwalByClassDay, siswaByClass, includeSaturdays) {
  ...
  while (current <= endDate) {
    if (isSchoolDay(current, includeSaturdays)) {
  ...
  await batchInsertAbsensi(connection, absensiSiswaBatch, absensiGuruBatch);
  absensiSiswaBatch.length = 0;
  absensiGuruBatch.length = 0;
}

async function seedArchiveTables(connection, startDate, endDate) {
...
async function seedPengajuanBandingAbsen(connection, kelasIdsList) {
...
async function main() {
  ...
  try {
    ...
    const { mapelFinal, kelasNameSet, ruangCodeSet, maxGuruId, maxSiswaId } = await loadReferenceData(connection);

    console.log('Seeding jam_pelajaran...');
    await seedJamPelajaran(connection, JAM_CONFIG, CONFIG.includeSaturdays, CONFIG.tahunAjaran);

    console.log('Seeding ruang_kelas...');
    const ruangFinal = await seedRuangKelas(connection, CONFIG.classCount, ruangCodeSet);

    console.log('Seeding kelas...');
    const kelasFinal = await seedKelas(connection, CONFIG.classCount, runTag, kelasNameSet);

    console.log('Seeding users for guru...');
    const { guruMapel } = await seedGuruUsersAndData(
      connection,
      CONFIG.teacherCount,
      runTag,
      maxGuruId,
    ...
    const { siswaByClass } = await seedSiswaUsersAndData(
      connection,
      kelasFinal,
      CONFIG.studentsPerClass,
      runTag,
      maxSiswaId
    );

    console.log('Syncing mata_pelajaran with mapel...');
    await syncMataPelajaran(connection, mapelFinal);
    ...
    const targetDays = getTargetDays(CONFIG.includeSaturdays);
    const { jadwalFinal, kelasIdsList } = await seedJadwalAndRelated(
      connection,
      kelasFinal,
      mapelFinal,
      guruMapel,
    ...
    const jadwalByClassDay = buildJadwalByClassDay(jadwalFinal);
    await generateAbsensiData(
      connection,
      startDate,
      endDate,
      kelasFinal,
    ...
    await seedArchiveTables(connection, startDate, endDate);

    console.log('Seeding pengajuan_banding_absen...');
    await seedPengajuanBandingAbsen(connection, kelasIdsList);
