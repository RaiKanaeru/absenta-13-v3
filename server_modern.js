...
// ================================================
// IMPORTANT: All time functions use Intl.DateTimeFormat API for reliable timezone handling
// This ensures correct behavior regardless of server's system timezone
 ...
 */
function formatWIBTime(date = null) {
 ...
 */
function getDayNameWIB(date = null) {
    const targetDate = date || new Date();
        ...
        if (!isUpdate || nip !== undefined) {
            if (!nip || typeof nip !== 'string') {
                ...
                try {
                    ...
                    console.error('NIP validation error details:', {
        ...
        if (!isUpdate || username !== undefined) {
            if (!username || typeof username !== 'string') {
                ...
                try {
                    ...
                    console.error('Username validation error details:', {
                        message: error.message,
                        code: error.code,
                        errno: error.errno
                    });
        ...
        if (email !== undefined && email !== null && email !== '') {
            if (typeof email !== 'string' || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
                ...
                try {
                    ...
                    console.error('Email validation error details:', {
                        message: error.message,
                        code: error.code,
                        errno: error.errno
                    });
        ...
        if (mapel_id !== undefined && mapel_id !== null && mapel_id !== '' && mapel_id !== 0) {
            if (!Number.isInteger(Number(mapel_id)) || Number(mapel_id) <= 0) {
                ...
                try {
                    ...
                    console.error('Mapel validation error details:', {
                        message: error.message,
                        code: error.code,
                        errno: error.errno
                    });
...
async function validateSiswaPayload(body, { isUpdate = false, excludeStudentId = null, excludeUserId = null } = {}) {
    const errors = [];
    const { nis, nama, username, email, kelas_id, jenis_kelamin, jabatan } = body;
...
app.post('/api/auth/login', async (req, res) => {
    try {
        ...
        if (user.role === 'guru') {
            ...
            if (guruData.length === 0) {
                console.log('❌ Auth login failed: Guru data not found');
                return res.status(401).json({ error: 'Guru data not found' });
            }
            ...
            if (siswaData.length === 0) {
                console.log('❌ Auth login failed: Student data not found');
                return res.status(401).json({ error: 'Student data not found' });
            }

            additionalData = {
                siswa_id: siswaData[0].id_siswa,
                nis: siswaData[0].nis,
                kelas: siswaData[0].nama_kelas || null
            };
        }
...
app.post('/api/admin/siswa', authenticateToken, requireRole(['admin']), async (req, res) => {
    try {
        const { nis, nama, kelas_id, username, password, jabatan, nomor_telepon_siswa } = req.body;
        ...
        if (nomor_telepon_siswa) {
            const [existingPhone] = await global.dbPool.execute(
                'SELECT id FROM siswa WHERE nomor_telepon_siswa = ?',
                [nomor_telepon_siswa]
            );
            if (existingPhone.length > 0) {
        ...
        // Create siswa record
        await global.dbPool.execute(
            'INSERT INTO siswa (nis, nama, kelas_id, user_id, jabatan, telepon_orangtua, nomor_telepon_siswa, status) VALUES (?, ?, ?, ?, ?, ?, ?, "aktif")',
            [nis, nama, kelas_id, userResult.insertId, jabatan || 'Sekretaris Kelas', null, nomor_telepon_siswa || null]
        );

        // The siswa record is already created above
...
app.post('/api/admin/guru', authenticateToken, requireRole(['admin']), async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            console.log('🔍 Mapel ID value:', mapelIdValue);
...
app.put('/api/admin/guru/:id', authenticateToken, requireRole(['admin']), async (req, res) => {
    ...
    try {
        ...
        console.log('🔍 Existing guru data:', guru);

        // Validasi payload dengan error handling yang lebih baik
        try {
            console.log('🔍 Starting validation for update...');
